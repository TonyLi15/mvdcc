...
// template <typename Protocol> class Transaction {
//   public:
//     uint32_t thread_id = 0;
//     Worker<Protocol> &worker;

//     Transaction(Worker<Protocol> &worker)
//         : thread_id(worker.get_id()), worker(worker),
...
//     void abort() {
//         protocol->abort();
//         protocol->set_new_ts(worker.get_abort_boosted_ts(),
//                              worker.get_smallest_ts(),
//                              worker.get_largest_ts());
//     }
...
//     template <typename Record>
//     Result prepare_record_for_update(Record *&rec_ptr,
//                                      typename Record::Key rec_key) {
//         // rec_ptr points to data in writeset copied from db
//         rec_ptr = reinterpret_cast<Record *>(
//             protocol->update(get_id<Record>(), rec_key.get_raw_key()));
//         return rec_ptr == nullptr ? Result::ABORT : Result::SUCCESS;
//     }
...
//     template <typename Record>
//     Result prepare_record_for_write(Record *&rec_ptr,
//                                     typename Record::Key rec_key) {
//         // rec_ptr points to allocated record
//         rec_ptr = reinterpret_cast<Record *>(
//             protocol->write(get_id<Record>(), rec_key.get_raw_key()));
//         return rec_ptr == nullptr ? Result::ABORT : Result::SUCCESS;
//     }
